...
export default async function CustomersPage({
  ...
  const [customers, total] = await Promise.all([
    Customer.find(filter)
      ...
      .limit(pageSize)
      .lean<Array<{ _id: unknown; name: string; phone?: string; address?: string }>>(),
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400">Manage customer records.</p>
        </div>
        <div className="flex gap-2">
          <form className="flex gap-2" action="/dashboard/customers" method="get">
            <input
              name="q"
              defaultValue={q}
              placeholder="Search..."
              ...
              <tbody>
                {customers.map((c) => (
                  <tr key={String(c._id)} className="border-b border-zinc-900/60">
                    ...
                    <td className="py-2 text-zinc-300">{c.address || "-"}</td>
                    <td className="py-2">
                ...
                {!customers.length ? (
                  <tr>
                    <td className="py-6 text-sm text-zinc-400" colSpan={4}>
                      No customers yet.
                    </td>
                  </tr>
                ) : null}
            ...
            <div className="mt-4 flex items-center justify-between gap-3">
              {page <= 1 ? (
                ...
                <Link
                  ...
                  href={`/dashboard/customers?q=${encodeURIComponent(q)}&page=${page - 1}`}
                >
                  Prev
                </Link>
              )}
